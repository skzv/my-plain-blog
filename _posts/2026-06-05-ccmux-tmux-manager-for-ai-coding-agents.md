...
tmux provides the session persistence and ccmux is the view over it; [Tailscale](https://tailscale.com/) gives the machines a network anywhere, and [Charm](https://charm.sh/) makes the TUI. ccmux reads the other machines' daemons directly, and attaching is a real attach, SSH from the laptop or Mosh from the phone.
